...
def match_keypoints(descriptors1, descriptors2, threshold = 0.7):
    ...
    matches = []
    for index_d1, desc1 in enumerate(descriptors1):
        distances = np.linalg.norm(desc1-descriptors2, axis=1)
        indexes = [index for index in range(0, len(distances))]
        dict_dist_index = dict(zip(distances, indexes))
        distances_sorted = sorted(distances)

        while len(distances_sorted) > 1:
            #take ratio between first and second closest
            ratio = distances_sorted[0]/distances_sorted[1]
            #Keep it
            if ratio <= threshold:
                matches.append([index_d1, dict_dist_index[distances_sorted[0]]])
            distances_sorted.pop(0)
...
def refine_match(keypoints1, keypoints2, matches, reprojection_threshold = 10,
    ...
    for iteration in range(0, num_iterations):
        #Generate subset with 
        index = np.random.choice(matches.shape[0], int(0.5*len(matches)), replace=False) 
        subset_matches = matches[index]
        print(subset_matches)
        #Create homography
        W = []
        b = []
        for sub in subset_matches:
            #create W homogeneus
            kp1, kp2 = sub
            x, y = keypoints1[kp1][:2]
            x_, y_ = keypoints2[kp2][:2]
            W.append([x, 0, 1])
            W.append([0, 1, 1])
            b.append([x_, y_, 1])
