...
def form_initial_voxels(xlim, ylim, zlim, num_voxels):
    ...
    #Compute area prism
    vol_prism = abs(xlim[0]-xlim[1])*abs(ylim[0]-ylim[1])*abs(zlim[0]-zlim[1])
    vol_voxel = vol_prism/num_voxels
    voxel_size = np.cbrt([vol_voxel])[0]
    
    x_voxels = abs(xlim[0]-xlim[1])/voxel_size
    y_voxels = abs(ylim[0]-ylim[1])/voxel_size
    z_voxels = abs(zlim[0]-zlim[1])/voxel_size

    voxels = []

    for z in np.arange(zlim[0], zlim[1], voxel_size, dtype='float'):
        for y in np.arange(ylim[0], ylim[1], voxel_size, dtype='float'):
            for x in np.arange(xlim[0], xlim[1], voxel_size, dtype='float'):
                voxels.append([x, y, z])
                
    return np.array(voxels), voxel_size
...
def carve(voxels, camera):
    ...
    #Create image bounds
    xmin, ymin = 2*[0]
    ymax, xmax = camera.silhouette.shape
    for voxel in voxels:
        homogenieus_voxel = np.hstack((voxel, [1]))
        p2D = np.dot(camera.P, homogenieus_voxel)
        p2D /= p2D[-1]
        x, y, _ = p2D
        x = int(x)
        y = int(y)
