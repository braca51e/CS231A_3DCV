...
def compute_K_from_vanishing_points(vanishing_points):
    #TODO: Fill in this code
    #each row a.T*w
    #a.T = (v1u1+v2u2, v1u3+v3u1, v2u3+v3u2, v3u3) v3, u3 = 1
    v1 = vanishing_points[0]
    v2 = vanishing_points[1]
    v3 = vanishing_points[2]
    A = np.zeros((len(vanishing_points), 4))
    omega = np.zeros((3, 3))
    
    A[0] = [v1[0]*v2[0]+v1[1]*v2[1], v1[0]+v2[0], v1[1]+v2[1], 1.0]
    A[1] = [v1[0]*v3[0]+v1[1]*v3[1], v1[0]+v3[0], v1[1]+v3[1], 1.0]
    A[2] = [v3[0]*v2[0]+v3[1]*v2[1], v3[0]+v2[0], v3[1]+v2[1], 1.0]
    
    U, s, V_T = np.linalg.svd(A)
    w = V_T[-1, :]
    omega[0][0], omega[0][2] = w[0], w[1]
    omega[1][1], omega[1][2] = w[0], w[2]
    omega[2][0], omega[2][1], omega[2][2] = w[1], w[2], w[3]
    
    KT_inv = np.linalg.cholesky(omega)
    K = np.linalg.inv(KT_inv.T)
    #Ensure last row has a 1 K intrisic
    K /= K[2][2]
...
def compute_angle_between_planes(vanishing_pair1, vanishing_pair2, K):
    #TODO: Fill in this code
    vanishing_pair1 = np.append(vanishing_pair1, np.ones((len(vanishing_pair1), 1)), axis=1)
    vanishing_pair2 = np.append(vanishing_pair2, np.ones((len(vanishing_pair2), 1)), axis=1)
    v1_1 = vanishing_pair1[0]
    v2_1 = vanishing_pair1[1]
    v1_2 = vanishing_pair2[0]
    v2_2 = vanishing_pair2[1]

    l1 = np.cross(v1_1, v2_1)
    l2 = np.cross(v1_2, v2_2)
    
    omega = np.linalg.inv(np.dot(K, K.T))
    num = np.dot(np.dot(l1.T, np.linalg.inv(omega)), l2)
    den = np.sqrt(np.dot(np.dot(l1.T, np.linalg.inv(omega)), l1))*np.sqrt(np.dot(np.dot(l2.T, np.linalg.inv(omega)), l2))

    return np.arccos(num/den)*(180.0/3.1416)
...
def compute_rotation_matrix_between_cameras(vanishing_points1, vanishing_points2, K):
    #TODO: Fill in this code
    vanishing_points1 = np.append(vanishing_points1, np.ones((len(vanishing_points1), 1)), axis=1)
    vanishing_points2 = np.append(vanishing_points2, np.ones((len(vanishing_points2), 1)), axis=1)
    K_i = np.linalg.inv(K)
    
    D_1 = np.zeros((3, 3))
    D_1[:, 0] = np.dot(K_i, vanishing_points1[0])/np.linalg.norm(np.dot(K_i, vanishing_points1[0]))
    D_1[:, 1] = np.dot(K_i, vanishing_points1[1])/np.linalg.norm(np.dot(K_i, vanishing_points1[1]))
    D_1[:, 2] = np.dot(K_i, vanishing_points1[2])/np.linalg.norm(np.dot(K_i, vanishing_points1[2]))
    
    D_2 = np.zeros((3, 3))
    D_2[:, 0] = np.dot(K_i, vanishing_points2[0])/np.linalg.norm(np.dot(K_i, vanishing_points2[0]))
    D_2[:, 1] = np.dot(K_i, vanishing_points2[1])/np.linalg.norm(np.dot(K_i, vanishing_points2[1]))
    D_2[:, 2] = np.dot(K_i, vanishing_points2[2])/np.linalg.norm(np.dot(K_i, vanishing_points2[2]))
    
    return np.dot(D_2, np.linalg.inv(D_1))
